import{d as de,w as j,j as Le,U as L,V as f,a2 as w,a0 as ae,a1 as s,_ as e,$ as p,a5 as oe,k as a,u as b,a7 as V,W as M,a8 as le,a3 as U,f as u,c as S,n as se,G as r,F as X,a4 as K,a6 as $e}from"./vendor-vue-n6x0MXoC.js";import{G as Ae}from"./GuideLayout-BIMqwKL5.js";import{C as E}from"./CodePreviewer-CdyH-HNW.js";import{K as ie,L as De,N as Me,O as Ne,H as Te,G as ze}from"./vendor-antd-DQF42uR_.js";import{_ as ce}from"./index-82OMxN3c.js";import"./FlowDesignerTest-CVT-husl.js";import"./vendor-x6-B0tTI3Tl.js";const Ge={class:"code-drawer__header"},He={class:"code-drawer__header-left"},Re={class:"code-drawer__title"},Fe={class:"code-drawer__header-actions"},Oe={class:"code-drawer__font-size-label"},Pe={key:0,class:"code-drawer__search-bar"},We={class:"code-drawer__search-count"},Be={class:"code-drawer__pre"},Ie=["innerHTML"],je=10,Ve=24,Xe=de({__name:"CodeDrawer",props:{visible:{type:Boolean,default:!1},code:{default:""},language:{default:"text"},title:{default:"代码示例"},width:{default:600},showLineNumbers:{type:Boolean,default:!0}},emits:["update:visible","close"],setup(J,{expose:z,emit:G}){const x=J,H=G,y=u(!1),$=u(""),n=u("text"),A=u("代码示例"),i=u(600),g=u(!1),C=u(!0),_=u("idle"),k=u(!1),h=u(""),c=u([]),v=u(0),Y=u(),q=u(),m=u(13),ue=u(.8),Z=S(()=>$.value||x.code||""),R=S(()=>n.value||x.language||"text"),P=S(()=>{try{const t=R.value.toLowerCase(),d=Z.value;if(t==="json"){const o=JSON.parse(d);return JSON.stringify(o,null,2)}return d}catch{return Z.value}}),pe=S(()=>{const d=P.value.split(`
`),o=d.length,l=String(o).length;return d.map((N,D)=>{const T=String(D+1).padStart(l," "),F=ve(N,x.language),O=me(F,D);return x.showLineNumbers?`<span class="code-line"><span class="line-number">${T}</span><span class="line-content">${O}</span></span>`:`<span class="code-line"><span class="line-content">${O}</span></span>`}).join(`
`)}),fe=S(()=>({vue:"Vue",typescript:"TypeScript",ts:"TypeScript",javascript:"JavaScript",js:"JavaScript",json:"JSON",html:"HTML",css:"CSS",scss:"SCSS",less:"Less",text:"Text"})[R.value.toLowerCase()]||R.value.toUpperCase()),ge=S(()=>({vue:"#42b883",typescript:"#3178c6",ts:"#3178c6",javascript:"#f7df1e",js:"#f7df1e",json:"#292929",html:"#e34c26",css:"#264de4",scss:"#c6538c",less:"#1d365d"})[R.value.toLowerCase()]||"#999999"),he=S(()=>_.value==="success"?"已复制":_.value==="error"?"复制失败":"复制代码");function ve(t,d){const o=d.toLowerCase();let l=Q(t);return o==="json"?l=l.replace(/"([^"]+)":/g,'<span class="hl-key">"$1"</span>:').replace(/: "([^"]*)"/g,': <span class="hl-string">"$1"</span>').replace(/: (\d+\.?\d*)/g,': <span class="hl-number">$1</span>').replace(/: (true|false)/g,': <span class="hl-boolean">$1</span>').replace(/: (null)/g,': <span class="hl-null">$1</span>'):o==="vue"||o==="html"?l=l.replace(/(&lt;\/?)([\w-]+)/g,'$1<span class="hl-tag">$2</span>').replace(/([\w-]+)=/g,'<span class="hl-attr">$1</span>=').replace(/="([^"]*)"/g,'="<span class="hl-string">$1</span>"'):o==="typescript"||o==="ts"||o==="javascript"||o==="js"?l=l.replace(/\b(const|let|var|function|return|if|else|for|while|class|interface|type|import|export|from|default|async|await)\b/g,'<span class="hl-keyword">$1</span>').replace(/'([^']*)'/g,`'<span class="hl-string">$1</span>'`).replace(/"([^"]*)"/g,'"<span class="hl-string">$1</span>"').replace(/\/\/(.*)/g,'<span class="hl-comment">//$1</span>'):(o==="css"||o==="scss"||o==="less")&&(l=l.replace(/([.#]?[\w-]+)\s*\{/g,'<span class="hl-selector">$1</span> {').replace(/([\w-]+):/g,'<span class="hl-attr">$1</span>:').replace(/: ([^;]+);/g,': <span class="hl-value">$1</span>;')),l}function Q(t){const d={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"};return t.replace(/[&<>"']/g,o=>d[o])}function me(t,d){if(!h.value||c.value.length===0)return t;const l=c.value[v.value]===d,N=Q(h.value),D=new RegExp(N,"gi");return t.replace(D,T=>`<mark class="${l?"search-match search-match--current":"search-match"}">${T}</mark>`)}async function be(){try{await navigator.clipboard.writeText(P.value),_.value="success",setTimeout(()=>{_.value="idle"},2e3)}catch(t){console.error("复制失败:",t),_.value="error",setTimeout(()=>{_.value="idle"},2e3)}}function W(){y.value=!1,H("update:visible",!1),H("close"),g.value=!1,k.value=!1,h.value="",c.value=[]}function _e(t){try{return JSON.parse(t),"json"}catch{}return t.includes("<template>")&&t.includes("<script")?"vue":t.trim().startsWith("<!DOCTYPE")||t.trim().startsWith("<html")?"html":/^\s*[\w-]+\s*\{[\s\S]*\}\s*$/.test(t)?"css":/\b(interface|type|enum|namespace)\b/.test(t)?"typescript":/\b(const|let|var|function|class|import|export)\b/.test(t)?"javascript":"text"}function ye(t){$.value=t.code,t.language?n.value=t.language:(n.value=_e(t.code),console.log("[CodeDrawer] 自动检测语言:",n.value)),A.value=t.title||"代码示例",i.value=t.width||600,y.value=!0}function we(){y.value=!1,W()}function xe(){g.value=!g.value}function Ce(){C.value=!C.value}function ee(){m.value<Ve&&(m.value+=1)}function ne(){m.value>je&&(m.value-=1)}function ke(){m.value=13}function B(t){(t.ctrlKey||t.metaKey)&&(t.key==="+"||t.key==="=")&&(t.preventDefault(),ee()),(t.ctrlKey||t.metaKey)&&t.key==="-"&&(t.preventDefault(),ne()),(t.ctrlKey||t.metaKey)&&t.key==="0"&&(t.preventDefault(),ke())}function re(){k.value=!k.value,k.value?se(()=>{var t;(t=Y.value)==null||t.focus()}):(h.value="",c.value=[])}function Se(){if(!h.value){c.value=[];return}const t=P.value.split(`
`),d=[];t.forEach((o,l)=>{o.toLowerCase().includes(h.value.toLowerCase())&&d.push(l)}),c.value=d,v.value=0,d.length>0&&I(d[0])}function te(){c.value.length!==0&&(v.value=(v.value+1)%c.value.length,I(c.value[v.value]))}function Ee(){c.value.length!==0&&(v.value=v.value===0?c.value.length-1:v.value-1,I(c.value[v.value]))}function I(t){se(()=>{const d=q.value;if(!d)return;const l=d.querySelectorAll(".code-line")[t];l&&l.scrollIntoView({behavior:"smooth",block:"center"})})}return j(h,()=>{Se()}),j(()=>x.visible,t=>{y.value=t||!1,t||(g.value=!1,C.value=!0,k.value=!1,h.value="",c.value=[],_.value="idle")}),j(y,t=>{t?document.addEventListener("keydown",B):(document.removeEventListener("keydown",B),g.value=!1,C.value=!0,k.value=!1,h.value="",c.value=[],_.value="idle",m.value=13,setTimeout(()=>{$.value="",n.value="text",A.value="代码示例"},300))}),Le(()=>{document.removeEventListener("keydown",B)}),z({openDrawer:ye,closeDrawer:we}),(t,d)=>{const o=L("a-button"),l=L("a-tooltip"),N=L("a-divider"),D=L("a-input"),T=L("a-button-group"),F=L("a-drawer");return f(),w(F,{open:y.value,width:g.value?"100%":i.value,class:ae({"code-drawer--fullscreen":g.value}),placement:"right",closable:!1,bodyStyle:{padding:0,height:"100%",display:"flex",flexDirection:"column"},onClose:W},{default:s(()=>[e("div",Ge,[e("div",He,[e("span",Re,p(A.value),1),e("span",{class:"code-drawer__language-tag",style:oe({background:ge.value})},p(fe.value),5)]),e("div",Fe,[a(l,{title:"搜索代码 (Ctrl+F)"},{default:s(()=>[a(o,{type:"text",size:"small",onClick:re},{icon:s(()=>[a(b(ie))]),_:1})]),_:1}),a(l,{title:"缩小字体 (Ctrl+-)"},{default:s(()=>[a(o,{type:"text",size:"small",onClick:ne,disabled:m.value<=10},{icon:s(()=>[a(b(De))]),_:1},8,["disabled"])]),_:1}),e("span",Oe,p(m.value)+"px",1),a(l,{title:"增大字体 (Ctrl++)"},{default:s(()=>[a(o,{type:"text",size:"small",onClick:ee,disabled:m.value>=24},{icon:s(()=>[a(b(Me))]),_:1},8,["disabled"])]),_:1}),a(N,{type:"vertical",style:{margin:"0 4px"}}),a(l,{title:C.value?"折叠代码":"展开代码"},{default:s(()=>[a(o,{type:"text",size:"small",onClick:Ce},{icon:s(()=>[(f(),w(V(C.value?"ShrinkOutlined":"ArrowsAltOutlined")))]),_:1})]),_:1},8,["title"]),a(l,{title:g.value?"退出全屏":"全屏查看"},{default:s(()=>[a(o,{type:"text",size:"small",onClick:xe},{icon:s(()=>[(f(),w(V(g.value?"FullscreenExitOutlined":"FullscreenOutlined")))]),_:1})]),_:1},8,["title"]),a(l,{title:he.value},{default:s(()=>[a(o,{type:"text",size:"small",onClick:be},{icon:s(()=>[(f(),w(V(_.value==="success"?"CheckOutlined":"CopyOutlined")))]),_:1})]),_:1},8,["title"]),a(l,{title:"关闭 (ESC)"},{default:s(()=>[a(o,{type:"text",size:"small",onClick:W},{icon:s(()=>[a(b(Ne))]),_:1})]),_:1})])]),k.value?(f(),M("div",Pe,[a(D,{ref_key:"searchInputRef",ref:Y,value:h.value,"onUpdate:value":d[0]||(d[0]=O=>h.value=O),placeholder:"搜索代码...",size:"small",onKeyup:[le(te,["enter"]),le(re,["esc"])]},{prefix:s(()=>[a(b(ie))]),suffix:s(()=>[e("span",We,p(c.value.length>0?`${v.value+1}/${c.value.length}`:"0/0"),1)]),_:1},8,["value"]),a(T,{size:"small"},{default:s(()=>[a(o,{onClick:Ee,disabled:c.value.length===0},{default:s(()=>[a(b(Te))]),_:1},8,["disabled"]),a(o,{onClick:te,disabled:c.value.length===0},{default:s(()=>[a(b(ze))]),_:1},8,["disabled"])]),_:1})])):U("",!0),e("div",{class:ae(["code-drawer__content",{"code-drawer__content--collapsed":!C.value}])},[e("pre",Be,[e("code",{ref_key:"codeRef",ref:q,class:"code-drawer__code",style:oe({fontSize:`${m.value}px`,lineHeight:ue.value}),innerHTML:pe.value},null,12,Ie)])],2)]),_:1},8,["open","width","class"])}}}),Ke=ce(Xe,[["__scopeId","data-v-59fc6667"]]),Ue=`import type { CanvasConfig } from '../../../views/types/flow.types'\r
\r
/**\r
 * 默认画布配置\r
